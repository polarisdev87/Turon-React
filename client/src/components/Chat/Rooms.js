                ...
                <ul>
                    {
                        this.props.rooms.map((v) => (
                            <li class={`contact ${this.props.activeRoom === v.id ? 'active' : ''}`}>
                                <a 
                                    onClick={(e) => {
                                        e.preventDefault();
                                    ...
                                    <div className="img-letters revert">
                                    {
                                        ...
                                        ? _.upperCase(_.first(_.split(v.teacherName, ' ')[0]) + _.first(_.split(v.teacherName, ' ')[1]))
                                        : _.upperCase(_.first(_.split(v.studentName, ' ')[0]) + _.first(_.split(v.studentName, ' ')[1]))
                                    }</div>
                                    <div className="meta">
                                        <p className="name d-flex justify-content-between">
                                            <div>
                                                ...
                                                {
                                                    this.props.activeRoom !== v.id && v.message && v.message.unread !== 0 && (
                                                        <small style={{fontSize: '1rem'}} className="text-warning ml-2 mb-0">({v.message.unread})</small>
                                                    )
                                                }
                                            </div>
                                            ...
                                            <small>
                                                {
                                                    v.message && (
                                                        <Moment fromNow ago>{v.message.ts}</Moment>
                                                    )
                                                }
                                            </small>
                                        </p>
