    ...
    async handleLinkedIn(data){
        try{
            ...
            try{
                ...
                const res = await axios.get(`${baseUrl}/linkedin?code=${data.code}`);
            ...
            <ul>
                <li>
                    <GoogleLogin
                        autoLoad={false}
                        render={renderProps => (
                            <button onClick={renderProps.onClick} className="no-button" style={{color: '#DC4E41'}}>
                               <img src={require('./../images/google.png')} width="48" height="48" alt="" />
                            </button>
                        )}
                ...
                <li>
                    <LinkedIn
                        clientId="86af1b1sawv1pr"
                        onFailure={this.handleLinkedIn.bind(this)}
                        onSuccess={this.handleLinkedIn.bind(this)}
                        redirectUri={`${window.location.origin}/linkedin_redirect`}
                        className="no-button btn-linkedin"
                        >
                            
                            <img width="48" height="48" src={require('./../images/linkedin.svg')} alt="" />
                    </LinkedIn>
                </li>
            </ul>
        )
